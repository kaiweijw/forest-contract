...
namespace Forest.Whitelist
{
    public partial class WhitelistContractTests : WhitelistContractTestBase
    {
        ...
        public async Task<Hash> CreateWhitelistTest()
        {
            ...
            {
                var tagInfo =
                    await WhitelistContractStub.GetTagInfoByHash.CallAsync(
                        CalculateId(whitelistId, _projectId, "INFO1"));
        ...
        public async Task CreateWhitelist_Duplicate()
        {
            ...
            var executionResult = await WhitelistContractStub.CreateWhitelist.SendWithExceptionAsync(
                new CreateWhitelistInput()
                {
                    ProjectId = projectId,
                    ExtraInfoList = new ExtraInfoList()
                    {
        ...
        public async Task AddExtraInfoTest()
        {
            ...
            {
                var exception = await WhitelistContractStub.GetExtraInfoByTag.CallWithExceptionAsync(
        ...
        {
            var whitelistId = await AddExtraInfoTest_WithAddress();
            var executionResult = await WhitelistContractStub.RemoveTagInfo.SendWithExceptionAsync(
                new RemoveTagInfoInput()
                {
                    WhitelistId = whitelistId,
                    ProjectId = _projectId,
        ...
        public async Task ResetWhitelist()
        {
            ...
            var executionResult = await UserWhitelistContractStub.ResetWhitelist.SendWithExceptionAsync(
                new ResetWhitelistInput
                {
                    WhitelistId = whitelistId,
                    ProjectId = _projectId
                });
            ...
            {
                var exception = await WhitelistContractStub.GetExtraInfoIdList.CallWithExceptionAsync(
                    new GetExtraInfoIdListInput()
                    {
                        ProjectId = _projectId,
                        WhitelistId = whitelistId
